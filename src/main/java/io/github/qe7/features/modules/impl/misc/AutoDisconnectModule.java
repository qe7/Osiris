...
public class AutoDisconnectModule extends Module {
	private final EnumSetting<Mode> mode = new EnumSetting<Mode>("Mode", Mode.Health);
        	...
        	case Player:
        		for(Entity entity : mc.theWorld.loadedEntityList) {
        			if(entity instanceof EntityPlayer) {
        				...
        				if(!Osiris.getInstance().getRelationManager().isFriend(entityPlayer.username) 
        						&& !Osiris.getInstance().getRelationManager().isEnemy(entityPlayer.username)
        						&& !this.onlyEnemies.getValue()) {
        					mc.theWorld.sendQuittingDisconnectingPacket();
        					break;
        				}
