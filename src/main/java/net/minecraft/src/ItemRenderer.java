...
public class ItemRenderer {
    ...
    public void renderItemInFirstPerson(float par1) {
        ...
        if (itemstack != null && itemstack.itemID == Item.map.shiftedIndex) {
            ...
            int itemCount = entityplayersp.getItemInUseCount();
            ...
            final RenderItemEvent event = new RenderItemEvent(enumaction1, f11, itemCount);
            Osiris.getInstance().getEventBus().post(event);

            enumaction1 = event.getAction();
            f11 = event.getSwingProgress();
            itemCount = event.getUseItemCount();
